...
from vmraid.utils import add_to_date, get_bench_relative_path, now
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("bench --site {site} execute vmraid.db.get_database_size")
		self.assertEqual(self.returncode, 0)
		self.assertIsInstance(float(self.stdout), float)

		# test 2: execute a command expecting an errored output as local won't exist
		self.execute("bench --site {site} execute vmraid.local.site")
		...
		self.execute("""bench --site {site} execute vmraid.bold --kwargs '{{"text": "DocType"}}'""")
		self.assertEqual(self.returncode, 0)
	...
	def test_restore(self):
		...
		self.execute(
			"bench --site {another_site} execute vmraid.utils.backups.fetch_latest_backups",
			site_data,
		)
	...
	def test_get_bench_relative_path(self):
		bench_path = vmraid.utils.get_bench_path()
	...
	def test_vmraid_site_env(self):
		...
		self.execute("bench execute vmraid.ping")
		self.assertEqual(self.returncode, 0)
